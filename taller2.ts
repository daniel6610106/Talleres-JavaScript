...
function algo (boleano: boolean, texto: string, numero:number)  {
...
const imprimir: () => String = () => "Hola ADSO";
...
const edadPasada = (edad: number): string => `la edad pasada es ${edad - 1}`;
...
const multiplicar = (factor1: number, factor2:number): number => factor1 * factor2;
...
const modular = (num1: number, num2: number): number => {
